# Authors: William Lam and Deji Akomolafe (VMware by Broadcom)
...
$vmotionNotifyLogFile = "<Path-to-where-you-want-to-save-logfiles\WSFC-vMotion-App-Notification.logs"
...
#############################################################################################################################################################################################################
        ...
        $message = "$(Get-Date -Format "dddd MM/dd/yyyy HH:mm:ss") vMotion Notification has ended. Un-pausing WSFC Node now"
        Write-Host -ForegroundColor Yellow $message
        # We write a entry into the Log File to show that we resumed cluster services on the Node.
        $message | Out-File -Append -FilePath $vmotionNotifyLogFile
        Resume-ClusterNode
